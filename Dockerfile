...
RUN rm /etc/apt/sources.list && \
    ...
    python3 python-is-python3 python3-pip certbot nodejs sqlite3 logrotate dnsutils redis-tools redis-server perl tar git jq curl wget zip unzip && \
    apt autoremove --purge -y && \
    apt autoclean -y && \
    apt clean -y && \
    npm i -g npm yarn && \
...
# fancyindex
    ...
    mv /src/Nginx-Fancyindex-Theme/Nginx-Fancyindex-Theme-dark /fancyindex && \
