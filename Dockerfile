...
COPY --from=build /src/opentelemetry-cpp-contrib/instrumentation/nginx/otel_ngx_module.so /usr/local/nginx/modules/otel_ngx_module.so
COPY --from=build /usr/local/lib/libngx_module.so                                         /usr/local/nginx/modules/libngx_module.so
