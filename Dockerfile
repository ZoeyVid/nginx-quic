...
RUN apk upgrade --no-cache -a && \
    apk add --no-cache ca-certificates tzdata tini lua-resty-core lua-resty-lrucache luajit pcre2 zlib brotli zstd libssl3 libcrypto3 geoip libmaxminddb-libs libldap && \
    ln -s /usr/local/nginx/sbin/nginx /usr/local/bin/nginx
