...
RUN apk upgrade --no-cache -a && \
    apk add --no-cache ca-certificates build-base libffi-dev cargo openssl-dev && \
...
COPY --from=zoeyvid/nginx-quic:latest /usr/local/lib/otel_ngx_module.so               /usr/local/lib/otel_ngx_module.so
